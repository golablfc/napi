...
# -*- coding: utf-8 -*-
"""
NapiProjekt – pobieranie napisów (wersja z filtrowaniem wierszy bez czasu)
"""
...
_duration_pat = re.compile(r"(\d{1,2}):(\d{2}):(\d{2})")
    ...
    def _detail(self, url):
        ...
        for pg in range(1, MAX_PAGES + 1):
            ...
            for row in rows:
                ...
                if len(tds) < 5:
                    continue
                # kolumna 2 (index 2) = długość HH:MM:SS
                dur_txt = tds[2].get_text(strip=True)
                m = _duration_pat.fullmatch(dur_txt)
                if not m:
                    self.log.debug(f"   ✗ pomijam id={h} – brak czasu")
                    continue
