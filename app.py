...
def manifest():
    ...
    return jsonify({
        ...
        "description": "Pobiera napisy z NapiProjekt.pl z informacją o czasie trwania",
...
def get_subtitles(content_type, imdb_id_with_params):
    ...
    try:
        ...
        if not item.get('title') and not item.get('tvshow') and base_imdb_id.startswith('tt'):
            try:
                ...
                if omdb_response.get('Response') == 'True':
                    ...
                    # Pobierz czas trwania filmu z OMDB jeśli dostępny
                    runtime = omdb_response.get('Runtime', '')
                    if runtime and 'min' in runtime:
                        try:
                            item['movie_duration'] = int(runtime.split()[0])
                        except:
                            pass
        ...
        for sub in found_subtitles:
            ...
            # Buduj szczegółową etykietę z informacjami
            duration_info = f" | Czas: {sub.get('duration_text', '?')}"
            fps_info = f" | FPS: {sub.get('fps', '?')}"
            downloads_info = f" | Pobrania: {sub.get('downloads', '?')}"
            
            # Dodaj ocenę dopasowania czasowego jeśli mamy dane
            if 'movie_duration' in item and 'duration_sec' in sub:
                movie_duration_min = item['movie_duration']
                sub_duration_min = round(sub['duration_sec'] / 60)
                time_diff = abs(movie_duration_min - sub_duration_min)
                
                if time_diff <= 2:  # Idealne dopasowanie (±2 minuty)
                    match_icon = " ✅"
                elif time_diff <= 5:  # Dobre dopasowanie (±5 minut)
                    match_icon = " ~"
                else:  # Znaczna różnica
                    match_icon = " ⚠️"
                
                duration_info += f"{match_icon} (film: {movie_duration_min}min)"
            
            label = f"NapiProjekt - PL{duration_info}{fps_info}{downloads_info}"
            ...
            stremio_subtitles.append({
                ...
                "label": label,
                "meta": {
                    "duration": sub.get('duration_text'),
                    "fps": sub.get('fps'),
                    "downloads": sub.get('downloads'),
                    "added_date": sub.get('added_date'),
                    "translator": sub.get('translator'),
                    "size": sub.get('size')
                }
            })
...
if __name__ == '__main__':
    logger.info("Starting Stremio NapiProjekt addon with duration support")
